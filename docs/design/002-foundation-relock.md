...
pepshop, nx-monorepo, spa-bff, cra-legacy). The reviews converged on four foundation gaps,
...
- **gate-integrity** (improvement) — the first flagship lens: a deterministic **gate inventory**
  ...
  **latent** (coverage collected but no local threshold — the cra-legacy case; commitlint unwired).
...
first embodiment — they already caught one real miss (cra-legacy's script-less sonar job) during this
rework.

## Deferred (recorded, not lost)

- `metrics` (ingest pepshop's `loop-metrics.mjs` output) — next after the gate lens beds in.
- Harvesting the two real AI-review CI jobs (pepshop claude-review, nx-monorepo `ai review`) into pack
