...
> from nx-monorepo, spa-bff, and cra-legacy. Tool name **`clothaid`** (npm-available).
...
Cursor natively). The same skeleton was hand-ported into **nx-monorepo** and **spa-bff**
(team repos: `AGENTS.md` + `.github/copilot-instructions.md` pointer, reuse-first + `INVENTORY.md`,
org-tooling leash, "Done =" gates) and touches **cra-legacy**. There is concrete demand, which
promises a **"portable kit … a setup CLI"** plus **measured** delivery gains against a deadline.
...
- **Sibling-path corpus**: a `sources.json` manifest pointing at local siblings — `../pepshop`
  (primary/frontrunner), `../nx-monorepo`, `../spa-bff`, `../cra-legacy`. No vendoring/
  submodules; works today because all repos are siblings.
- **Dogfooding validation loop** (core to development): regenerate pepshop's real artifacts from
  ...
  pnpm-workspace TanStack (pepshop), Nx monorepo Next.js (nx-monorepo), single-repo SPA + Express BFF
  (spa-bff), CRA + Apollo + husky/lint-staged (cra-legacy).
...
and nx-monorepo/spa-bff's `.github/copilot-instructions.md` pattern, generalized:
...
the contract's "Working rules"/"leash" section; the nx-monorepo/spa-bff org-tooling block is the proof this
generalizes.
...
- **Detected** (auto): infer likely constraints from the scan (GitLab + no `gh` → org VCS; husky
  commitlint → enforced commit format; no MCP config → assume none) — the nx-monorepo/spa-bff leash is exactly
  this shape.
...
nx-monorepo/spa-bff pain): map lists files that no longer exist? documented commands still resolve? leash
...
version. This is how the tool learns from pepshop/nx-monorepo/spa-bff continuing to evolve.
...
- **Opt-in AI-review CI job** (`--ci`): the `AI-judgment → advisory` MR/PR review that nx-monorepo/spa-bff
...
- **Shape coverage**: run against nx-monorepo (Nx monorepo), spa-bff (single-repo SPA + BFF), cra-legacy (CRA) —
