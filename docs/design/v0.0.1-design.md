...
> Founding design document (v0.0.1). Distilled from the pepshop frontrunner workflow plus lessons
...
pepshop (the frontrunner project) has, over 122 sessions, converged a **locked agentic
...
**context economy on a measured loop**: pepshop proved context is ~85% of loop spend and that
...
- **New repo**: `~/projects/clothaid` (own release cadence; must not couple an agent-agnostic
  product to pepshop's history).
- **Sibling-path corpus**: a `sources.json` manifest pointing at local siblings — the pepshop
  frontrunner plus the three private team repos. No vendoring/submodules; works today because all
  repos are siblings.
- **Dogfooding validation loop** (core to development): regenerate pepshop's real artifacts from
  ...
  pnpm-workspace TanStack (pepshop), Nx monorepo Next.js, single-repo SPA + Express BFF, and
...
The distilled, **versioned** encoding of the pepshop-derived standard: contract templates, the
...
it was set up against (drift is then computable). Ships a default "pepshop-standard" pack;
...
generated pointer/adapter files per agent target** — exactly pepshop's `CLAUDE.md → @AGENTS.md`
...
Re-runnable audit generalizing pepshop's `freshness-audit` skill + R1 map-staleness (the recurring
...
time/story vs a baseline — generalizing pepshop's A4 instrument (`scripts/loop-metrics.mjs` →
...
version. This is how the tool learns from pepshop and the team repos continuing to evolve.
...
pre-commit`, `correctness → pre-push` (format/types/lint over the working tree) — via
  `.githooks/` + `core.hooksPath` (pepshop's multi-agent-safe, git-level pattern). **No keys.**
...
Encodes pepshop's session protocol: `start` opens a session with the right context loaded (state →
...
tokens), flags bloat, and proposes the **skill-extraction move** (map → skill) that cut pepshop's
context −52% forever. Context is ~85% of loop spend (pepshop's own finding) — this is the measured
...
promotes cross-project ones into the shared pack. pepshop's failure-modes skill, networked.
...
- Pin deps exact (pepshop convention); the tool holds itself to the standard it installs
...
- Knowledge pack v1 (default pepshop-standard) + maturity rubric (§3a).
...
- **Corpus reproduction**: run against `../pepshop` and diff generated vs hand-built `AGENTS.md`/
