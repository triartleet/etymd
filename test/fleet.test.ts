...
describe("fleet add — the corp perimeter", () => {
  ...
  it("lets a non-corp remote through on the personal path", async () => {
    ...
    // The guard must refuse corp hosts ONLY — an ordinary remote registers normally. The URL
...
describe("fleet add — the manifest records no remote", () => {
  it("PINNED: never writes a remote URL into the tracked manifest", async () => {
    ...
    const manifestPath = await writeHub([], { local: { machineProfile: "corp", dirs: {} } })
